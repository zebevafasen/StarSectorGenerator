...
function PlanetsSection({ expanded, onToggle, selectedSystem }) {
  return (
    <div>
      <SectionToggleButton
        icon={<Disc size={12} />}
...
function Tooltip({ tooltip, displayStarInfo }) {
  if (!tooltip.show || !displayStarInfo) return null;
...
function InspectorPanel({ gridSize, systems, selectedCoords, setSelectedCoords }) {
  ...
  return (
    <aside className="w-96 bg-slate-900 border-l border-slate-800 flex flex-col shrink-0 shadow-xl z-20">
      ...
      <Tooltip tooltip={tooltip} displayStarInfo={displayStarInfo} />
    </aside>
  );
}
