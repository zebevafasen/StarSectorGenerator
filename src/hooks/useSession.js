...
export function useSession() {
  ...
  const [sectorCoords, setSectorCoords] = useState(initialSession?.sectorCoords || { q: 0, r: 0 });
  const [viewMode, setViewMode] = useState(initialSession?.viewMode || 'local');
  const [universe, setUniverse] = useState(initialSession?.universe || {});
  const [showGalaxyPOIs, setShowGalaxyPOIs] = useState(initialSession?.showGalaxyPOIs ?? true);
  ...
  }, [gridSize, systems, generatorSettings, showLeftSidebar, showRightSidebar, viewState, sectorCoords, viewMode, universe, showGalaxyPOIs]);
